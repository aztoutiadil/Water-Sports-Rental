...
if (isset($conn)) {
    try {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $title = "Jet Ski: " . $jet_ski['brand'] . " " . $jet_ski['model'];
        } else {
        ...
        $res_stmt = $conn->prepare("
            ...
            WHERE r.equipment_type = 'jet_ski' AND r.equipment_id = ? 
        ...
        $maint_stmt = $conn->prepare("
            SELECT * FROM jet_ski_maintenance 
            WHERE jet_ski_id = ? 
            ORDER BY date DESC
        ");
...
// Use dummy data if no jet ski found or no database connection
if ($jet_ski === null) {
    // Create dummy jet ski data
    $jet_ski = [
        'id' => $id,
        'brand' => 'Example Brand',
        'model' => 'Sample Model',
        'year' => 2023,
        'registration_number' => 'JS-' . sprintf("%04d", $id),
        'hourly_rate' => 75.00,
        'daily_rate' => 300.00,
        'status' => 'available',
        'last_maintenance_date' => date('Y-m-d', strtotime('-30 days')),
        'next_maintenance_date' => date('Y-m-d', strtotime('+30 days')),
        'notes' => 'This is sample data as the actual jet ski could not be found.',
        'image_url' => ''
    ];
}
?>
    ...
    <div class="main-content">
        <div class="top-bar">
            ...
            <div>
                ...
                <a href="index.php?page=jet-skis/edit&id=<?php echo $jet_ski['id']; ?>" class="btn btn-primary ms-2">
            ...
            <div class="col-md-4">
                <div class="card mb-4">
                    <div class="card-body text-center">
                        <?php if (!empty($jet_ski['image_url'])): ?>
                            <img src="<?php echo htmlspecialchars($jet_ski['image_url']); ?>" alt="<?php echo htmlspecialchars($jet_ski['model']); ?>" class="detail-image">
                        <?php else: ?>
                            <div class="bg-light d-flex align-items-center justify-content-center mb-3" style="height: 200px; border-radius: 8px;">
                                <i class="bi bi-water text-secondary" style="font-size: 4rem;"></i>
                            </div>
                        <?php endif; ?>
                        
                        <h3 class="mb-1"><?php echo htmlspecialchars($jet_ski['brand'] ?? 'Unknown Brand'); ?> <?php echo htmlspecialchars($jet_ski['model'] ?? 'Unknown Model'); ?></h3>
                        <p class="text-muted mb-3"><?php echo htmlspecialchars($jet_ski['registration_number'] ?? 'No Registration'); ?></p>
                        
                        <?php
                        $statusClass = '';
                        switch($jet_ski['status'] ?? 'unavailable') {
                            case 'available':
                        ...
                        ?>
                        <span class="badge <?php echo $statusClass; ?> mb-3"><?php echo ucfirst($jet_ski['status'] ?? 'Unavailable'); ?></span>
                        
                        <div class="d-grid gap-2">
                            <?php if (($jet_ski['status'] ?? '') === 'available'): ?>
                                <a href="index.php?page=reservations/create&jet_ski_id=<?php echo $jet_ski['id']; ?>" class="btn btn-success">
                                    <i class="bi bi-calendar-plus"></i> Book Now
                                </a>
                            <?php endif; ?>
                            
                            <?php if (($jet_ski['status'] ?? '') !== 'maintenance'): ?>
                            ...
                            <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteModal">
                                <i class="bi bi-trash"></i> Delete Jet Ski
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header bg-white">
                        <h5 class="mb-0">Jet Ski Details</h5>
                    </div>
                    <div class="card-body">
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Brand</div>
                            <div class="col-7"><?php echo htmlspecialchars($jet_ski['brand'] ?? 'N/A'); ?></div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Model</div>
                            <div class="col-7"><?php echo htmlspecialchars($jet_ski['model'] ?? 'N/A'); ?></div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Year</div>
                            <div class="col-7"><?php echo htmlspecialchars($jet_ski['year'] ?? 'N/A'); ?></div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Registration</div>
                            <div class="col-7"><?php echo htmlspecialchars($jet_ski['registration_number'] ?? 'N/A'); ?></div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Hourly Rate</div>
                            <div class="col-7">$<?php echo htmlspecialchars($jet_ski['hourly_rate'] ?? '0.00'); ?></div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Daily Rate</div>
                            <div class="col-7">$<?php echo htmlspecialchars($jet_ski['daily_rate'] ?? '0.00'); ?></div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Last Maintenance</div>
                            <div class="col-7"><?php echo isset($jet_ski['last_maintenance_date']) ? date('M d, Y', strtotime($jet_ski['last_maintenance_date'])) : 'N/A'; ?></div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-5 detail-label">Next Maintenance</div>
                            <div class="col-7"><?php echo isset($jet_ski['next_maintenance_date']) ? date('M d, Y', strtotime($jet_ski['next_maintenance_date'])) : 'N/A'; ?></div>
                        </div>
                        <?php if (!empty($jet_ski['notes'])): ?>
                            <div class="mt-3">
                                <div class="detail-label mb-2">Notes</div>
                                <div><?php echo nl2br(htmlspecialchars($jet_ski['notes'] ?? '')); ?></div>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="reservations-tab-pane" role="tabpanel" aria-labelledby="reservations-tab" tabindex="0">
                                <?php if (empty($reservation_history)): ?>
                                    <div class="alert alert-info">
                                        <i class="bi bi-info-circle me-2"></i> No reservation history found for this jet ski.
                                    </div>
                                <?php else: ?>
                            ...
                            <div class="tab-pane fade" id="maintenance-tab-pane" role="tabpanel" aria-labelledby="maintenance-tab" tabindex="0">
                                <?php if (empty($maintenance_history)): ?>
                                    <div class="alert alert-info">
                                        <i class="bi bi-info-circle me-2"></i> No maintenance history found for this jet ski.
                                    </div>
                                <?php else: ?>
                                    <div class="table-responsive">
                                        <table class="table table-hover">
    ...
    <div class="modal fade" id="maintenanceModal" tabindex="-1" aria-labelledby="maintenanceModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="index.php?page=jet-skis/addMaintenance" method="POST">
                    <input type="hidden" name="id" value="<?php echo $jet_ski['id'] ?? 0; ?>">
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this jet ski?</p>
                    <p class="text-danger">This action cannot be undone. All related maintenance records will also be deleted.</p>
                    
                    <div class="alert alert-warning">
                        <i class="bi bi-exclamation-triangle me-2"></i> 
                        Deleting a jet ski with existing reservations may cause inconsistencies in the system.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    <a href="index.php?page=jet-skis/delete&id=<?php echo $jet_ski['id'] ?? 0; ?>" class="btn btn-danger">
                        <i class="bi bi-trash"></i> Delete Jet Ski
                    </a>
                </div>
            </div>
