...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: index.php?page=jet-skis/create');
    exit;
}

// Validate required fields
$required_fields = ['brand', 'model', 'year', 'registration_number', 'hourly_rate', 'daily_rate', 'status'];
$errors = [];
...
if (isset($conn)) {
    try {
        $stmt = $conn->prepare("
            INSERT INTO jet_skis (
                brand, model, year, registration_number, hourly_rate, 
                daily_rate, status, notes, last_maintenance_date, 
        ...
        if ($result) {
            $_SESSION['flash']['success'] = "Jet ski added successfully";
            header('Location: index.php?page=jet-skis');
            exit;
        } else {
    ...
    $_SESSION['flash']['success'] = "Jet ski added successfully (Simulation - no database connection)";
    header('Location: index.php?page=jet-skis');
    exit;
} 